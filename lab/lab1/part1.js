...
var query5=_.filter(bakedGoods,function(t){return t.price>4;});
...
===================== */
  console.log('Stretch Goal 1: menu with head');
  var printMenu2=function(food){
    for(i=0;i<Object.keys(food).length;i++){
      console.log(Object.keys(food)[i]);
      printMenu(Object.values(food)[i]);
    }
  };
...
===================== */
  console.log('Stretch Goal 2: template');
  var printMenu3=function(food){
        for(i=0;i<Object.keys(food).length;i++){
            var compiled = _.template("<%= name %> ... $ <%= price %>");
            console.log(compiled(food[i]));
          }
  };
  printMenu3(bakedGoods);
