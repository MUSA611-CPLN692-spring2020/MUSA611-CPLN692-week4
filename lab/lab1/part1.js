...
var query5 = _.filter(bakedGoods, function(bakedGood) {return bakedGood["price"] > 4});
...
var query6 = _.sortBy(bakedGoods, function(bakedGood) {return bakedGood["inventory"]});
...
var query7 = _.groupBy(bakedGoods, function(bakedGood) {return bakedGood["type"]});
...
function printMenu2(newBakedGoods) {
  _.each(newBakedGoods, function(newBakedGood, key)
  {
    console.log(key);
    _.each(newBakedGood, function (item)
    {
      var template = _.template("<% print(name + ' ... $' + price); %>");
