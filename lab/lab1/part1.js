...
// There is such thing as a chain
// _.chain(myData).filter((x) => (x > 3)).map((x) => (x * 2)).value();

var keys = _.keys(query7);

var newMenu = function(k) {
  console.log(k);
  var section = _.where(bakedGoods, {type: k});
  printMenu(section);
};

var groupedMenu = keys.map(newMenu);
