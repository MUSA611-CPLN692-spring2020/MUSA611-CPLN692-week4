...
var query5 = _.filter(bakedGoods, function(eachGood) {return eachGood.price > 4})
...
var printMenu2 = function(foodList) {
  _.each(foodList, function(food) {
    console.log(food[0].type)
    _.each(food, function(eachItem) {
      var printline = _.template("<%= foodname %> ... $ <%= foodvalue %>")
      console.log(printline({foodname: eachItem.name, foodvalue: eachItem.price}))
    })
